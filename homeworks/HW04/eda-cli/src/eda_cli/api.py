...
@app.post(
    ...
    tags=["quality"],
    summary="Полный набор флагов качества из CSV файла (новый эндпоинт для HW04)",
)
async def quality_flags_from_csv(file: UploadFile = File(...)):
    """
    Новый эндпоинт, который возвращает только флаги качества.
    Использует эвристики из HW03: обнаружение константных колонок и высокой кардинальности.
    
    Ответ соответствует формату из задания HW04:
    {
        "flags": {
            "too_few_rows": bool,
            "too_many_missing": bool,
            "has_constant_columns": bool,          # из HW03
            "has_high_cardinality_categoricals": bool  # из HW03
        }
    }
    """
    start = perf_counter()
    ...
    # Формируем ответ ТОЛЬКО с флагами (как в задании HW04)
    flags_response = {
    ...
    print(
        ...
        f"has_constant_columns={flags_response['has_constant_columns']} "
        f"has_high_cardinality={flags_response['has_high_cardinality_categoricals']} "
        f"latency_ms={latency_ms:.1f} ms"
    )
